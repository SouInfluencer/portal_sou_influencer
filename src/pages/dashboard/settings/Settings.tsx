...
import { User, Bell, Shield, AlertTriangle, Home, Building as Bank, BarChart as ChartBar } from 'lucide-react';
...
export function Settings() {
  ...
  const [formSuccess] = useState<string | null>(null);
  ...
  type TabType = 'personal' | 'address' | 'bank';
  ...
  const calculateProfileCompletion = (data: SettingsFormData): number => {
    const tabs: TabType[] = ['personal', 'address', 'bank'];
  ...
  const isTabComplete = (tab: TabType, data: SettingsFormData): boolean => {
    switch (tab) {
      case 'personal':
        return Boolean(data.firstName && data.lastName && data.cpf && data.birthDate && data.phone && data.birthDate);
      case 'address':
  ...
  const validateForm = () => {
    ...
    if (!formData.lastName.trim()) {
      errors.firstName = 'Sobrenone é obrigatório';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      // Proceed with form submission logic, such as API calls
            ...
            <div className="mt-6">
              ...
              {activeTab === 'notifications' && (
                  <NotificationSettings
                      formData={formData}
                      onNotificationChange={(type, setting) => handleNotificationChange(type, setting as "campaigns" | "messages" | "updates" | "marketing")}
                  />
              )}
